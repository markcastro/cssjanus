...
function CSSJanus() {
	...
	var
		...
		translateRegExp = new RegExp( '(translateX?\\(\\s*)(-?' + numPattern + '\\s*)('+ unitPattern + ')', 'gi' );
	...
	/**
	 * Calculate new Translate X position
	 *
	 * @private
	 * @param {string} match Matched property
	 * @param {string} pre Text before value
	 * @param {string} value X value
	 * @param {string} post Text after value
	 * @return {string} Inverted property
	 */
	function calculateNewTranslatePosition( match, pre, value, post ) {
		return pre + ( -1 *(Number( value )) ) + post;
	}
	...
	return {
		...
		transform: function( css, swapLtrRtlInUrl, swapLeftRightInUrl ) {
			...
			css = css
				...
				// CSS Transforms
				.replace( translateRegExp, calculateNewTranslatePosition );
